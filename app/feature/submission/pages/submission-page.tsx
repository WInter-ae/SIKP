...
import { requestSuratKesediaanApproval } from "~/lib/services/surat-kesediaan-api";
...
function SubmissionPage() {
  ...
  const documents = UPLOADABLE_DOCUMENT_TYPES
    .filter((type) => type !== "PROPOSAL_KETUA") // Proposal handled separately
    .map((type, index) => ({
      id: index + 1,
  ...
  const handleAjukanSuratKesediaan = async (memberId: string) => {
    try {
      const response = await requestSuratKesediaanApproval(memberId);
      if (response.success) {
        toast.success("Pengajuan surat kesediaan berhasil dikirim ke dosen.");
      } else {
          ...
          <div className="mb-8">
            ...
            {documents.map((document) => (
              <DocumentDropdown
                ...
                onSubmitRequest={handleAjukanSuratKesediaan}
                disabled={isSubmissionSubmitted}
              />
            ))}
          </div>
