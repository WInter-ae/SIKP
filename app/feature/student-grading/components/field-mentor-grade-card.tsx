...
import type { FieldMentorGrade } from "../types/index.d";

interface FieldMentorGradeCardProps {
  grade?: FieldMentorGrade;
}

export default function FieldMentorGradeCard({ grade }: FieldMentorGradeCardProps) {
  if (!grade) {
    return (
      <Card className="border-l-4 border-l-gray-400">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-xl">Penilaian Mentor Lapangan</CardTitle>
            <Badge variant="secondary">Belum Dinilai</Badge>
          </div>
        </CardHeader>
        <CardContent>
          <p className="text-gray-500 text-center py-8">
            Penilaian dari mentor lapangan belum tersedia
          </p>
        </CardContent>
      </Card>
    );
