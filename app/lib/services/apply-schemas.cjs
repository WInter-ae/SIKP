...
function addSchema(content, serviceName, genericType, schemaName) {
  ...
  const regexGet1 = new RegExp(
    "sikpClient\\\\.get<([^>]+" + genericType + "[^>]*)>\\\\(([^,]+)\\\\)",
    "g",
  );
  content = content.replace(
    regexGet1,
    "sikpClient.get<$1>($2, undefined, " + schemaName + ")",
  );

  // Replace get<Generic>(endpoint, params) -> get<Generic>(endpoint, params, Schema)
  const regexGet2 = new RegExp(
    "sikpClient\\\\.get<([^>]+" +
      genericType +
      "[^>]*)>\\\\(([^,]+),\\s*([^)]+)\\\\)",
    "g",
  ...
  content = content.replace(
    regexGet2,
    "sikpClient.get<$1>($2, $3, " + schemaName + ")",
  );

  // Replace post<Generic>(endpoint) -> post<Generic>(endpoint, undefined, Schema)
  const regexPost1 = new RegExp(
    "sikpClient\\\\.post<([^>]+" + genericType + "[^>]*)>\\\\(([^,]+)\\\\)",
    "g",
  );
  content = content.replace(
    regexPost1,
    "sikpClient.post<$1>($2, undefined, " + schemaName + ")",
  );

  // Replace post<Generic>(endpoint, body) -> post<Generic>(endpoint, body, Schema)
  const regexPost2 = new RegExp(
    "sikpClient\\\\.post<([^>]+" +
      genericType +
      "[^>]*)>\\\\(([^,]+),\\s*([^)]+)\\\\)",
    "g",
  ...
  content = content.replace(
    regexPut,
    "sikpClient.put<$1>($2, $3, " + schemaName + ")",
  );
...
files.forEach((file) => {
  ...
  if (
    file === "submission-api.service.ts" ||
    ...
    if (content !== original) {
      content =
        'import { TeamSchema, TeamMemberSchema } from "~/lib/schemas/api-schemas";\nimport { z } from "zod";\n' +
        content;
    }
    ...
    if (content !== original) {
      content =
        'import { TemplateSchema } from "~/lib/schemas/api-schemas";\nimport { z } from "zod";\n' +
        content;
    }
    ...
    if (content !== original) {
      content =
        'import { DosenProfileSchema } from "~/lib/schemas/api-schemas";\nimport { z } from "zod";\n' +
        content;
    }
    ...
    content = addSchema(
      ...
      file,
      "MahasiswaProfile",
      "MahasiswaProfileSchema",
    );
    if (content !== original) {
      content =
        'import { MahasiswaProfileSchema } from "~/lib/schemas/api-schemas";\nimport { z } from "zod";\n' +
        content;
    }
  }
